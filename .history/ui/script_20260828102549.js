...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // CLI-Auswertung über das neue --open Flag
    try {
        const cliMatches = await window.__TAURI__.cli.getMatches();
        
        // Greift gezielt auf das Flag "open" zu
        if (cliMatches && cliMatches.args && cliMatches.args.open && cliMatches.args.open.value) {
            
            // Da wir ein explizites Flag nutzen, kommen hier GARANTIERTE Bildpfade an!
            // Ein Filtern von System-Flags ist nicht mehr nötig.
            const uebergebenePfade = cliMatches.args.open.value.map(p => String(p).trim());
