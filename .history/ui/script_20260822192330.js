...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            //vorschauImg.src = new URL('./batch-icon.png', window.location.href).href;
            // 1. Hole den Pfad der aktuellen HTML-Seite und hänge das Bild an
            const bildPfad = new URL('./batch-icon.png', window.location.href).pathname;
            // 2. Konvertiere den Pfad in eine sichere Tauri-Asset-URL
            vorschauImg.src = window.__TAURI__.core.convertFileSrc(bildPfad);
