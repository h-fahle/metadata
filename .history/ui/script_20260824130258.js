...
//const tooltip = document.querySelector('.tooltip-boxX');

// Falls du hier deine Dateinamen einfügen willst:
// const pfadArray = ["/img/auto.png", "/img/haus.png"];
// tooltip.textContent = pfadArray.map(p => p.split('/').pop()).join('\n');

// Berechne die Position bei jeder Mausbewegung
container.addEventListener('mousemove', (e) => {
  const abstandVomCursor = 15; // Pixel Platz zwischen Cursor und Tooltip
  // e.clientX und e.clientY sind die aktuellen Maus-Koordinaten im Sichtfeld
//  tooltip.style.left = (e.clientX - abstandVomCursor) + 'px';
...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            // vorschauImg.title = extrakt.join('\n');

            const fileTtBox = document.getElementById("file-tt-box");
            fileTtBox.innerText = dateiListe.join('\n');
