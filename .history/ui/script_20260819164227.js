...
// Der Observer merkt jede Höhenänderung der Statusleiste
const resizeObserver = new ResizeObserver(entries => {
  for (let entry of entries) {
    // Die echte, aktuelle Höhe auslesen
    const hoehe = entry.contentRect.height;
    // Den Abstand des Bodys dynamisch anpassen
    document.body.style.paddingBottom = hoehe + 'px';
  }
});
// Beobachtung starten
// resizeObserver.observe(statusleiste);
...
async function ladeOrdnerBilder(pfad) {
    try {
        ...
        ausgabe.innerHTML = "<span style='color: red;'>Fehler beim Laden des Ordnerinhalts: " + fehler + "</span>";
        bilderListe = [pfad];
        aktuellerIndex = 0;
    }
}
...
async function zeigeMaps(event) {
    ...
    try {
        ...
        ausgabe.innerHTML = "<span style='color: red;'>Fehler bei öffnen Maps: " + fehler + "</span>";
    }
}
