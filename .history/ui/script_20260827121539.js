...
async function exifSchreiben(ereignis) {
    if (ereignis) ereignis.preventDefault(); // Verhindert Seiten-Reset

    // Im Einzelmodus nutzen wir 'aktuellerBildPfad', im Stapelmodus die gesamte 'bilderListe'
    const istStapel = (stapelmodus > 0);
    const pfadeFuerRust = istStapel ? bilderListe : [aktuellerBildPfad];

    if (pfadeFuerRust.length === 0 || pfadeFuerRust[0] === "") return;

    zeigeStatus("Speichere Daten über ExifTool...");

    // Wir lesen das dynamische Filename-Feld aus, falls es existiert (Einzelmodus)
    ...
    const daten = {
        dateipfade: pfadeFuerRust, // Übergibt das Array an Rust
        stapelmodus: istStapel,    // Übergibt das Flag an Rust
        filename: filenameWert,
    ...
    try {
        // Wir schicken das gesamte Paket an die Rust-Funktion
        const antwort = await invoke('schreibe_bild_metdaten', daten);

        zeigeStatus(antwort);

        // Nach dem Speichern die UI aktualisieren
        if (!istStapel) {
            // Im Einzelmodus lesen wir das Bild neu aus, um die Änderungen zu sehen
            await exifAuslesen();
        } else {
            zeigeStatus("Stapelverarbeitung für " + pfadeFuerRust.length + " Bilder abgeschlossen.");
        }
