...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            let extrakt = bilderListe.slice(0, 10);
            if (bilderListe.length > 10) {
                extrakt.push('...');
            }
            // vorschauImg.title = extrakt.join('\n');

            const vorschauImgCont = document.getElementById("vorschau-bild-container");
             vorschauImgCont.dataset.tooltipListe = extrakt.join('\n');
