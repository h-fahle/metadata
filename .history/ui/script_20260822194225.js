...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            // 1. Hole den reinen Pfadnamen deiner aktuellen HTML-Datei im Webview
            // (Entfernt das "http://127.0.0" und lässt nur den Ordnerpfad übrig)
            const aktuellerOrdnerPfad = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/'));

            // 2. Erzeuge den exakten Pfad zum Bild innerhalb des UI-Ordners
            const bildDateiname = "batch-icon.png";
            const vollständigerPfad = `${aktuellerOrdnerPfad}/${bildDateiname}`;

            // 3. Konvertiere es mit Tauri in die sichere "asset://localhost/..." URL
            vorschauImg.src = window.__TAURI__.core.convertFileSrc(vollständigerPfad);

            console.log(vorschauImg.src)
        }
    } catch (fehler) {
