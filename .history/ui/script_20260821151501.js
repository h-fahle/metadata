...
async function waehleUndLadeBilder() {
    try {
        ...
        if (auswahl) {
            ...
            bilderListe = await window.__TAURI__.core.invoke('hole_ordner_bilder', { dateipfad: startBild });
...
function blaettern(richtung) {
    ...
    if (richtung > 0) {
        aktuellerIndex = (aktuellerIndex + 1) % dateiliste.length;
    } else {
        aktuellerIndex = (aktuellerIndex - 1 + dateiliste.length) % dateiliste.length;
    }
