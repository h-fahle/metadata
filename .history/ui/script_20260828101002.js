...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // CLI-Auswertung für mehrere Dateien oder Ordner
    try {
        ...
        if (cliMatches && cliMatches.args && cliMatches.args.pfade && cliMatches.args.pfade.value) {
            // Tauri gibt uns hier ein Array von Strings zurück
            const uebergebenePfade = cliMatches.args.pfade.value.map(p => String(p));
            
            if (uebergebenePfade.length === 1) {
                // ==========================================================
                // FALL A: Genau EIN Pfad wurde übergeben (Datei oder Ordner)
                // ==========================================================
                const einzelPfad = uebergebenePfade[0];
                
                // Wir nutzen dein waehleUndLadeBilder-Konzept:
                // Wenn der Pfad ein Ordner ist, verhält sich die Logik wie dein Ordner-Button.
                // Da JS nicht direkt prüfen kann, ob es ein Ordner ist, fragen wir Rust
                // indirekt über deine Funktion 'hole_ordner_bilder' ab.
                ...
                if (testBilder.length > 0) {
                    // Wenn der übergebene Pfad eine Datei war, liefert Rust die Geschwisterbilder.
                    // Das ermitteln wir, indem wir prüfen, ob der Pfad exakt in der Liste existiert.
                    const dateiIndex = testBilder.indexOf(einzelPfad);

                    if (dateiIndex !== -1) {
                        // Es ist eine einzelne DATEI
                        zeigeStatus("Datei über CLI geöffnet: " + einzelPfad);
                        aktuellerBildPfad = einzelPfad;
                        bilderListe = testBilder;
                        aktuellerIndex = dateiIndex;
                        stapelmodus = 0; // Einzelmodus
                        await exifAuslesen();
                    } else {
                        // Es ist ein ORDNER (Rust hat alle Bilder im Ordner eingelesen)
                        zeigeStatus("Ordner über CLI geöffnet: " + einzelPfad);
                        bilderListe = testBilder;
                        stapelmodus = testBilder.length === 1 ? 0 : 2; // Stapelmodus Ordner
                        ...
                        if (stapelmodus === 0) {
                            ...
                            // UI für Stapelmodus aktualisieren (dein bestehender Code)
                            triggereStapelmodusUI(einzelPfad);
                        }
                    }
                }
            } else if (uebergebenePfade.length > 1) {
                // ==========================================================
                // FALL B: MEHRERE Bildpfade wurden übergeben
                // ==========================================================
                zeigeStatus(`${uebergebenePfade.length} Bilder über CLI geöffnet.`);
                bilderListe = uebergebenePfade;
                stapelmodus = 1; // Stapelmodus mehrere Dateien
                
                // Berechne den Ordnerpfad des ersten Bildes zur Anzeige im UI
                const erstespild = uebergebenePfade[0];
                const dirPath = erstespild.substring(0, Math.max(erstespild.lastIndexOf('/'), erstespild.lastIndexOf('\\')));
                
                triggereStapelmodusUI(dirPath);
            }
        }
    } catch (fehler) {
        console.error("CLI-Auswertungsfehler:", fehler);
    }

    // Kleine Hilfsfunktion, um dein bestehendes UI-Layout für den Stapelmodus zu aktivieren
    function triggereStapelmodusUI(ordnerPfad) {
        ...
        // Nutzt deine dynamische UI-Umschaltung für die Dropdown-Anzeige
...
async function dateiAuswählen() {
    try {
        ...
        zeigePopup(fehler, "Dialog-Fehler", "error")
...
async function ladeOrdnerBilder(pfad) {
    try {
        ...
        zeigePopup(fehler, "Fehler", "error")
        zeigeStatus(fehler);
    }
}
