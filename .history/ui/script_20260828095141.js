...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // CLI-Auswertung starten
    try {
        // Fragt das Tauri-CLI-Plugin nach den übergebenen Argumenten
        const cliMatches = await window.__TAURI__.cli.getMatches();
        
        // Prüfen, ob das Argument "bild" befüllt ist und einen Wert hat
        if (cliMatches && cliMatches.args && cliMatches.args.bild && cliMatches.args.bild.value) {
            const uebergebenerPfad = String(cliMatches.args.bild.value);
            
            zeigeStatus("Datei über CLI geöffnet: " + uebergebenerPfad);
            
            // 1. Den globalen App-Zustand auf diese Datei setzen
            aktuellerBildPfad = uebergebenerPfad;
            stapelmodus = 0; // Einzelmodus erzwingen

            // 2. Deine bestehende Funktion nutzen, um die restlichen Bilder des Ordners einzulesen
            await ladeOrdnerBilder(uebergebenerPfad);

            // 3. Die EXIF-Metadaten dieser Datei auslesen und im Formular anzeigen
            await exifAuslesen();
        }
    } catch (fehler) {
