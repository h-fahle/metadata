...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            // Die ersten 10 Bilder im Tooltip anzeigen:
            let extrakt = bilderListe.slice(0, 20);
            if (bilderListe.length >20) {
                extrakt.push('...');
            }
            const dateiListe = extrakt.map(pfad => pfad.split('/').pop());
            ...
            let dateiFeld = document.getElementById('filename');

            // 1. Erstelle ein datalist-Element
            const datalist = document.createElement('datalist');
            datalist.id = "dynamischeListe";
            // 2. Befülle die Liste mit den Optionen
            bilderListe.forEach(text => {
                const option = document.createElement('option');
                option.value = text;
                datalist.appendChild(option);
            });
            // 3. Hänge die Liste an das Dokument an
            document.body.appendChild(datalist);
            // 4. Verknüpfe das Input-Feld mit der Liste
            dateiFeld.setAttribute('list', 'dynamischeListe');
...
async function exifSchreiben(ereignis) {
    ...
    if (auswahlTyp === 'ordner') {
        ...
        daten.forEach(pfad => {
        const option = document.createElement('option');
        option.textContent = pfad.split('/').pop();
        ...
        // 'daten' ist hier der einzelne Dateiname oder Pfad
        input.value = daten.split('/').pop(); 

        // Hier kannst du Änderungen des Nutzers abfangen
        input.addEventListener('input', (e) => {
        console.log("Neuer Dateiname:", e.target.value);
        });
