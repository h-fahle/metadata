...
async function waehleUndLadeBilder(isFolder = false, selectMultiple = false) {
    try {
        ...
        if (stapelmodus > 0) {
            ...
            try {
                // 1. Hole den echten absoluten Ordnerpfad aus Rust
                // (Gibt z.B. "C:\DeinProjekt\src-tauri\resources" oder ähnlich zurück)
                const basisPfad = await window.__TAURI__.core.invoke("get_ui_path");

                // 2. Verbinde den Basispfad mit dem Bildnamen
                // Wir nutzen ein einfaches Trennzeichen, Tauri bereinigt doppelte Slashes automatisch
                const absoluterBildPfad = `${basisPfad}/batch-icon.png`;

                // 3. In die korrekte asset:// URL umwandeln
                const sichereTauriUrl = window.__TAURI__.core.convertFileSrc(absoluterBildPfad);

                // 4. Dem Bild zuweisen
                vorschauImg.src = sichereTauriUrl;
                
                console.log("Erfolgreiche URL:", sichereTauriUrl);
            } catch (error) {
