...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // CLI-Auswertung für mehrere Dateien oder Ordner (Bereinigt für "tauri dev")
    try {
        ...
        if (cliMatches && cliMatches.args && cliMatches.args.pfade && cliMatches.args.pfade.value) {
            ...
            // 1. Array einlesen und alle "tauri dev"-Flags (beginnend mit -) sowie leere Einträge herausfiltern!
            const uebergebenePfade = cliMatches.args.pfade.value
                .map(p => String(p).trim())
                .filter(p => p !== "" && !p.startsWith("-")); // Filtert --color, always, -- etc. heraus
            
            // Erst fortfahren, wenn nach dem Filtern überhaupt Pfade übrig bleiben
            if (uebergebenePfade.length > 0) {
                ...
                if (uebergebenePfade.length === 1) {
                    ...
                    const erstesBild = uebergebenePfade[0];
    ...
    // Hilfsfunktion für die Stapelmodus-Oberfläche (bleibt gleich)
