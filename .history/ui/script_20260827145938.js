...
async function exifSchreiben(ereignis) {
    ...
    try {
        ...
        if (!istStapel) {
            ...
            // NEU: Ein echtes OS-Popup für den Stapelmodus anzeigen
            // NATIVES POPUP ÜBER DEN GLOBALEN TAURI-SCOPE AUFRUFEN:
            await window.__TAURI__.dialog.message(
                `Die Metadaten wurden erfolgreich in ${pfadeFuerRust.length} Bilder geschrieben.`, 
                { 
                    title: 'Stapelverarbeitung abgeschlossen', 
                    kind: 'info' // Zeigt das standardmäßige Info-Icon des Betriebssystems
                }
            );
