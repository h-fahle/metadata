...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.__TAURI__) {
        ...
        const footer = document.getElementById('statusleiste');
        const main = document.getElementById('main-container');

        if (footer) {
            // getBoundingClientRect().bottom gibt den exakten Abstand von oben 
            // bis zur Unterkante des Footers in Pixeln an.
            const totalHeight = Math.ceil(footer.getBoundingClientRect().bottom);
            const fixedWidth = 1000; // deine gewünschte Breite
        ...
        if (cliMatches && cliMatches.args && cliMatches.args.pfade && cliMatches.args.pfade.value) {
            ...
            if (uebergebenePfade.length > 0) {
                if (uebergebenePfade.length === 1) {
                    ...
                    // ==========================================================
                    // FALL B: MEHRERE Bildpfade (Stapelmodus)
                    // ==========================================================
...
async function exifSchreiben(ereignis) {
    ...
    try {
        ...
        if (!istStapel) {
            await exifAuslesen();
            zeigeStatus(`Die Metadaten wurden erfolgreich geschrieben und neu geladen.`);
        } else {
...
function zeigeStatus(nachricht) {
    statusText.innerText = nachricht;
}
