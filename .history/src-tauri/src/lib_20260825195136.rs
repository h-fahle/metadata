...
fn hole_vorschaubild(dateipfad: String) -> Result<String, String> {
    ...
    if extension == "tif" || extension == "tiff" {
        ...
        // 2. Eindeutigen Namen via Hash generieren
        let mut hasher = DefaultHasher::new();
        dateipfad.hash(&mut hasher);
        let dateiname = format!("thumb_{}.jpg", hasher.finish());

        // 3. Das Standard-Temp-Verzeichnis des Betriebssystems nutzen
        let temp_dir = std::env::temp_dir();
        let ziel_pfad = temp_dir.join(dateiname);

        // 4. Konvertieren, falls die Datei noch nicht im Temp-Ordner liegt
        if !ziel_pfad.exists() {
            img.save_with_format(&ziel_pfad, image::ImageFormat::Jpeg)
                .map_err(|e| format!("Konvertierungsfehler: {}", e))?;
        }

        // 5. Den echten System-Pfad zum konvertierten JPEG an JavaScript senden
