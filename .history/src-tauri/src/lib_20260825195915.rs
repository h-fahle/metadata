...
/* #[tauri::command]
fn hole_ordner_bilder(dateipfade: Vec<String>) -> Result<Vec<String>, String> {
    let gueltige_endungen = ["jpg", "jpeg", "png", "gif", "tif", "tiff", "webp", "avif", "heic", "heif"];
    let mut bilder = Vec::new();

    // Schleife über alle vom Frontend übergebenen Dateipfade
    for pfad_str in dateipfade {
        let p = std::path::Path::new(&pfad_str);
        
        // Prüfen, ob der Pfad eine existierende Datei ist
        if p.is_file() {
            if let Some(ext) = p.extension().and_then(|s| s.to_str()) {
                // Auf erlaubte Bild-Endungen prüfen
                if gueltige_endungen.contains(&ext.to_lowercase().as_str()) {
                    if let Some(gueltiger_pfad) = p.to_str() {
                        bilder.push(gueltiger_pfad.to_string());
                    }
                }
            }
        }
...
fn hole_vorschaubild(dateipfad: String) -> Result<String, String> {
    ...
    if extension == "tif" || extension == "tiff" {
        // Versuchen, das Bild zu öffnen. Wenn der Farbraum nicht unterstützt wird, fangen wir das ab!
        match image::open(pfad) {
            Ok(img) => {
                ...
                if !ziel_pfad.exists() {
                    if let Err(e) = img.save_with_format(&ziel_pfad, image::ImageFormat::Jpeg) {
                        println!("Konvertierungsfehler: {}", e);
                        return Ok("STANDARD".to_string()); // Fallback auf Originalpfad
                    }
                }
                return Ok(ziel_pfad.to_string_lossy().to_string());
            }
            Err(e) => {
                // Hier landen wir bei dem Fehler "Unknown(24)"
                println!("TIFF konnte nicht gelesen werden (Farbraum nicht unterstützt): {}", e);
                
                // OPTION 1: Einfach "STANDARD" zurückgeben, damit das Frontend versucht, es normal zu laden (wird ein leeres Bild-Icon)
                return Ok("STANDARD".to_string());
                
                // OPTION 2: Wenn du ein Standard-Icon für "Fehler" im Frontend hast, gib hier z.B. "FEHLER" zurück
                // return Ok("FEHLER".to_string());
            }
        }
    }

    // 6. Für alle normalen Bilder (JPG, PNG) das Signalwort für dein Frontend zurückgeben
