...
#[tauri::command]
async fn hole_vorschaubild(app: tauri::AppHandle, dateipfad: String) -> Result<String, String> {
    let pfad = std::path::Path::new(&dateipfad);
    ...
    if extension == "tif" || extension == "tiff" || extension == "heic" || extension == "heif" {
        ...
        if !ziel_pfad.exists() {
            ...
            let output = command.output().await.map_err(|e| format!("ImageMagick-Fehler: {}", e))?;
...
pub fn run() {
    tauri::Builder::default()
        .plugin(tauri_plugin_cli::init()) // Plugin registrieren
        .setup(|app| {
            // CLI-Argumente parsen
            if let Ok(matches) = app.cli().matches() {
                // String-Argument auslesen (-i oder --input)
                if let Some(input_arg) = matches.args.get("input") {
                    if let Some(file_path) = input_arg.value.as_str() {
                        println!("Datei soll geöffnet werden: {}", file_path);
                    }
                }
                
                // Flag auslesen (-v oder --verbose)
                if let Some(verbose_arg) = matches.args.get("verbose") {
                    if verbose_arg.occurrences > 0 {
                        println!("Erweiterte Protokollierung aktiv.");
                    }
                }
            }
            Ok(())
