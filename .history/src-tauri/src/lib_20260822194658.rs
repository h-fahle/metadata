...
#[tauri::command]
fn get_ui_path(app: AppHandle) -> Result<String, String> {
    // Holt den Pfad zum Ressourcen-Verzeichnis deiner App
...
#[tauri::command]
fn hole_ordner_bilder(dateipfad: String) -> Result<Vec<String>, String> {
    let pfad = std::path::Path::new(&dateipfad);
    let ordner = pfad
        .parent()
        .ok_or_else(|| "Ordner nicht gefunden".to_string())?;
