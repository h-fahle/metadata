...
fn hole_ordner_bilder(
    ...
    println!(">>> FEHLERSUCHE: Command gestartet!");
    let pfad = std::path::Path::new(&dateipfad);
    
    let ordner = if nutze_parent {
        pfad.parent().unwrap_or(pfad)
    } else {
        pfad // Nutze den übergebenen Pfad direkt
    };

    dbg!(pfad)
    
    // Prüfen, ob der Zielpfad existiert
    if !ordner.exists() || !ordner.is_dir() {
        return Err("Der Zielpfad ist kein gültiger Ordner.".to_string());
    }

    let mut bilder = Vec::new();

    if let Ok(eintraege) = fs::read_dir(ordner) {
