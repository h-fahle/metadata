...
fn hole_vorschaubild(app: tauri::AppHandle, dateipfad: String) -> Result<String, String> {
    ...
    if extension == "tif" || extension == "tiff" {
        // 1. TIF-Bild von der Festplatte öffnen
        let img = image::open(pfad).map_err(|e| format!("TIFF-Ladefehler: {}", e))?;

        // 2. Einen eindeutigen Dateinamen für das Cache-JPEG erzeugen (per Hash)
        let mut hasher = DefaultHasher::new();
        dateipfad.hash(&mut hasher);
        let dateiname = format!("thumb_{}.jpg", hasher.finish());

        // 3. Cache-Ordner der App ermitteln und erstellen
        let cache_dir = app.path().app_cache_dir().map_err(|e| e.to_string())?;
        std::fs::create_dir_all(&cache_dir).map_err(|e| e.to_string())?;
        let ziel_pfad = cache_dir.join(dateiname);

        // 4. Nur konvertieren, wenn die Cache-Datei nicht existiert (spart CPU-Leistung!)
        if !ziel_pfad.exists() {
            img.save_with_format(&ziel_pfad, image::ImageFormat::Jpeg)
                .map_err(|e| format!("Konvertierungsfehler: {}", e))?;
        }

        // 5. Den echten System-Pfad zur fertigen JPEG-Datei zurückgeben
        return Ok(ziel_pfad.to_string_lossy().to_string());
    }

    // Für funktionierende Standard-Formate (jpg/png) direkt den Pfad zurückgeben
    Ok(dateipfad)
}
