...
use base64::{Engine as _, engine::general_purpose};  // für den tiff-Wandler


#[tauri::command]
fn hole_vorschaubild(dateipfad: String) -> Result<String, String> {
    let pfad = std::path::Path::new(&dateipfad);
    let extension = pfad.extension()
        .and_then(|s| s.to_str())
        .unwrap_or("")
        .to_lowercase();
    ...
    if extension == "tiff" || extension == "tif" {
        ...
        let img = image::open(pfad)
            .map_err(|e| format!("TIFF-Ladefehler: {}", e))?;
