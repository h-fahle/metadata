use std::process::Command;
use std::fs;
use std::hash::{Hash, Hasher};
use std::collections::hash_map::DefaultHasher;
use tauri::Manager; // Wichtig für den Zugriff auf App-Pfade // für den tiff-Wandler


// Debug-Ausgabe:  dbg!(&cmd);


#[tauri::command]
fn hole_ordner_bilder(
    dateipfad: String, 
    gueltige_endungen: Vec<String>
) -> Result<Vec<String>, String> {

    let pfad = std::path::Path::new(&dateipfad);

    // Automatische Erkennung:
    let ziel_ordner = if pfad.is_dir() {
        pfad // Es ist bereits ein Ordner, nimm ihn direkt
    } else {
        // Es ist eine Datei -> extrahiere den Ordnerpfad darüber
        pfad.parent().ok_or_else(|| "Kein übergeordneter Ordner gefunden".to_string())?
    };

    let mut bilder = Vec::new();

    if let Ok(eintraege) = fs::read_dir(ziel_ordner) {
        for eintrag in eintraege.flatten() {
            let p = eintrag.path();
            if p.is_file() {
                if let Some(ext) = p.extension().and_then(|s| s.to_str()) {
                    if gueltige_endungen.contains(&ext.to_lowercase()) {
                        if let Some(pfad_str) = p.to_str() {
                            bilder.push(pfad_str.to_string());
                        }
                    }
                }
            }
        }
    }

    // Alphabetisch sortieren, damit die Reihenfolge stimmt
    bilder.sort();

    Ok(bilder)
}

#[tauri::command]
async fn hole_vorschaubild(dateipfad: String) -> Result<String, String> {
    let pfad = std::path::Path::new(&dateipfad);
    let extension = pfad.extension().and_then(|s| s.to_str()).unwrap_or("").to_lowercase();

    if extension == "tif" || extension == "tiff" || extension == "heic" || extension == "heif" {
        let mut hasher = DefaultHasher::new();
        dateipfad.hash(&mut hasher);
        let dateiname = format!("thumb_{}.jpg", hasher.finish());

        let temp_dir = std::env::temp_dir();
        let ziel_pfad = temp_dir.join(dateiname);

        if !ziel_pfad.exists() {
            // WICHTIG: Wir rufen "magick" direkt als globalen Systembefehl auf, KEIN Sidecar mehr!
            // Unter Windows/Linux sucht das System nun im globalen PATH des Nutzers.
            let mut cmd = Command::new("magick");
            cmd.args([
                &dateipfad,
                "-thumbnail", "130x>",
                &ziel_pfad.to_string_lossy()
            ]);

            // Wir führen den Befehl aus und prüfen, ob das Programm überhaupt existiert
            match cmd.output() {
                Ok(output) => {
                    if !output.status.success() {
                        let fehler_text = String::from_utf8_lossy(&output.stderr);
                        return Err(format!("ImageMagick-Fehler bei Wandlung: {}", fehler_text));
                    }
                }
                Err(e) => {
                    // Wenn das Programm "magick" nicht auf dem PC installiert ist, landen wir HIER
                    if e.kind() == std::io::ErrorKind::NotFound {
                        return Err("ImageMagick wurde auf diesem System nicht gefunden. Bitte installieren Sie ImageMagick, um TIF/HEIC-Vorschauen anzuzeigen.".to_string());
                    }
                    return Err(format!("Systemfehler beim Aufruf von ImageMagick: {}", e));
                }
            }
        }

        return Ok(ziel_pfad.to_string_lossy().to_string());
    }

    // Für funktionierende Standard-Formate (jpg/png) direkt das Signalwort zurückgeben
    Ok("STANDARD".to_string())
}

#[tauri::command]
fn lese_bild_metdaten(app_handle: tauri::AppHandle, dateipfad: String) -> Result<String, String> {
    // 1. Wir holen uns den Pfad zum internen Ressourcen-Ordner der installierten App
    let resource_dir = app_handle
        .path()
        .resource_dir()
        .map_err(|e| format!("Ressourcen-Ordner nicht gefunden: {}", e))?;

    // 2. Wir bauen den exakten Pfad zur mitgelieferten Konfigurationsdatei
    let config_path = resource_dir.join(".ExifTool_config");

    // 3. ExifTool-Prozess mit dem exakten Ressourcen-Pfad starten
    let output = Command::new("exiftool")
        .arg("-config")
        .arg(config_path)
        .arg("-fast2")
        .arg("-d")
        .arg("%d.%m.%Y")
        .arg("-c")
        .arg("%-.6f")
        .arg("-json")
        .arg("-FileName")
        .arg("-ImageSize")
        .arg("-CMB_Caption")
        .arg("-CMB_Editor")
        .arg("-MWG:all")
        .arg("-GPSPosition")
        .arg(&dateipfad)
        .output()
        .map_err(|e| format!("ExifTool-Fehler: {}", e))?;

    if output.status.success() {
        Ok(String::from_utf8_lossy(&output.stdout).into_owned())
    } else {
        Err(String::from_utf8_lossy(&output.stderr).into_owned())
    }
}

#[tauri::command]
fn schreibe_bild_metdaten(
    app_handle: tauri::AppHandle,
    dateipfade: Vec<String>, // Geändert auf Vec<String> für den Stapelmodus
    stapelmodus: bool,       // Neues Flag vom Frontend
    filename: String,
    caption: String,
    description: String,
    keywords: String,
    creator: String,
    copyright: String,
    editor: String,
    gps: String,
) -> Result<String, String> {
    // 1. Pfad zur mitgelieferten .ExifTool_config auflösen
    let config_path = app_handle
        .path()
        .resolve(".ExifTool_config", tauri::path::BaseDirectory::Resource)
        .map_err(|e| format!("Konfigurationsfehler: {}", e))?;

    // 2. Den ExifTool-Schreibbefehl vorbereiten
    let mut cmd = Command::new("exiftool");
    cmd.arg("-config");
    cmd.arg(config_path);
    cmd.arg("-m");
    cmd.arg("-IPTCDigest=new");
    cmd.arg("-charset");
    cmd.arg("utf8");
    cmd.arg("-overwrite_original"); 
    cmd.arg("-sep");
    cmd.arg(" ,"); // Definiert das Komma als Trennzeichen für Keywords

    if !stapelmodus {
        // ==========================================
        // EINZELMODUS: Alles 1:1 schreiben
        // ==========================================
        cmd.arg(format!("-FileName={}", filename));
        cmd.arg(format!("-CMB_Caption_Set={}", caption));
        cmd.arg(format!("-MWG:Description={}", description));
        cmd.arg(format!("-MWG:Keywords={}", keywords));
        cmd.arg(format!("-MWG:Creator={}", creator));
        cmd.arg(format!("-MWG:Copyright={}", copyright));
        cmd.arg(format!("-CMB_Editor_Set={}", editor));
        cmd.arg(format!("-GPSPosition={}", gps));
    } else {
        // ==========================================
        // STAPELMODUS: Bedingungen anwenden
        // ==========================================
        // HINWEIS: Dateiname wird im Stapelmodus ignoriert.

        if !caption.trim().is_empty() {
            cmd.arg(format!("-CMB_Caption_Set={}", caption));
        }
        
        // Exiftool-Syntax zum ANHÄNGEN (+=) nutzen:
        if !description.trim().is_empty() {
            cmd.arg("-TagsFromFile");
            cmd.arg("@"); // Symbolisiert die aktuelle Datei selbst
            // Syntax: Nimm die vorhandene Beschreibung, füge ein echtes Newline und den Text hinzu
            cmd.arg(format!("-MWG:Description<${{MWG:Description}}\n{}", description));
        }
        if !keywords.trim().is_empty() {
            // Fügt neue Keywords an die bestehende Keywords-Liste an
            cmd.arg(format!("-MWG:Keywords+={}", keywords)); 
        }
        if !creator.trim().is_empty() {
            cmd.arg(format!("-MWG:Creator={}", creator));
        }
        if !copyright.trim().is_empty() {
            cmd.arg(format!("-MWG:Copyright={}", copyright));
        }
        if !editor.trim().is_empty() {
            cmd.arg(format!("-CMB_Editor_Set={}", editor));
        }
        if !gps.trim().is_empty() {
            cmd.arg(format!("-GPSPosition={}", gps));
        }
    }

    // Zum Schluss ALLE ausgewählten Bilder an den Befehl anhängen
    // ExifTool kann beliebig viele Dateipfade auf einmal verarbeiten!
    for pfad in &dateipfade {
        cmd.arg(pfad);
    }

    // 3. Befehl ausführen
    let output = cmd.output().map_err(|e| format!("Schreibfehler: {}", e))?;

    if output.status.success() {
        Ok("Metadaten erfolgreich gespeichert!".to_string())
    } else {
        Err(String::from_utf8_lossy(&output.stderr).into_owned())
    }
}


#[tauri::command]
fn oeffne_maps_browser(app_handle: tauri::AppHandle, gps_string: String) -> Result<(), String> {
    let url = if gps_string.trim().is_empty() {
        "https://google.com/maps".to_string()
    } else {
        format!("https://www.google.com/maps/search/?api=1&query={}", gps_string)
    };
    
    // Direkt über das Rust-Backend im Standardbrowser des Nutzers öffnen
    use tauri_plugin_opener::OpenerExt;
    app_handle.opener().open_url(&url, None::<&str>)
        .map_err(|e| format!("Fehler beim Öffnen: {}", e))?;
        
    Ok(())
}

#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    tauri::Builder::default()
        .plugin(tauri_plugin_cli::init())
        .plugin(tauri_plugin_dialog::init())
        .plugin(tauri_plugin_shell::init())
        .setup(|app| {
            // 1. Versionsnummer aus der tauri.conf.json holen
            let version = app.package_info().version.to_string();
            // 2. Das Hauptfenster suchen
            if let Some(main_window) = app.get_webview_window("main") {
                // 3. Den Titel dynamisch für das Betriebssystem setzen
                let _ = main_window.set_title(&format!("Metadaten Editor [v{}]", version));
                //main_window.show().unwrap();
            }
            Ok(())
        })
        .on_page_load(|main_window, payload| {
            // Prüfen, ob die Seite fertig geladen ist (Finished)
            if payload.event() == tauri::webview::PageLoadEvent::Finished {
                // Das Fenster erst einblenden, wenn das HTML/CSS fertig ist
                let _ = main_window.show();
            }
        })
        .invoke_handler(tauri::generate_handler![
            hole_ordner_bilder,
            lese_bild_metdaten,
            schreibe_bild_metdaten,
            hole_vorschaubild,
            oeffne_maps_browser
        ])
        .run(tauri::generate_context!())
        .expect("Fehler beim Starten der Tauri-Anwendung");
}
